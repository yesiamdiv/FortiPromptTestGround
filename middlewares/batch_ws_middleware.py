...
After the eval node fires (node_name == "eval"), this middleware reads
strategy_context["current_chunk_results"] and loops through every record,
emitting the SAME existing events in the SAME order as the automatic middleware:

    broadcast_attack_generated    — existing event, existing frontend handler
    broadcast_defence_response    — existing event, existing frontend handler
    broadcast_evaluation_complete — existing event, existing frontend handler
    broadcast_turn_completed      — existing event, existing frontend handler
    evaluation_stats_updated      — existing event, existing frontend handler
...
class BatchWSMiddleware(BaseMiddleware):
    ...
    async def after_step(
        ...
        """
        After the eval node: replay existing single-item WS events for every
        record in current_chunk_results — strictly sequentially.

        For all other nodes: emit the same routing progress broadcast as
        AutomaticWSMiddleware (router node).
        """
        if node_name == "router":
        ...
        try:
            ...
            if not chunk_results:
                warn("BatchWSMiddleware: no chunk_results to broadcast")
                return

            step("BatchWSMiddleware: broadcasting chunk", items=len(chunk_results))

            # Running success counter for stats (mirrors AutomaticWSMiddleware logic)
            running_successful = context.get("successful_iterations", 0)
            total_processed = context.get("total_processed", 0)
            # total_processed is already updated by BatchWrapperNode before middleware runs
            # so we back-calculate where this chunk started
            chunk_start_total = total_processed - len(chunk_results)
            dataset_total = len(context.get("dataset_prompts", []))

            # ── Sequential broadcast loop ──
            for local_idx, record in enumerate(chunk_results):
                ...
                # 1. broadcast_attack_generated (existing event)
                ...
                # 2. broadcast_defence_response (existing event)
                ...
                # 3. broadcast_evaluation_complete + broadcast_turn_completed (existing events)
                if self.middleware_config.get("broadcast_evaluations") and evaluation:
                    try:
                        ...
                        # 4. evaluation_stats_updated (existing event — same as AutomaticWSMiddleware)
                ...
                debug("BatchWSMiddleware: item broadcast complete", index=global_index)

            # Progress bar update after chunk (existing event via broadcast_run_progress)
            try:
