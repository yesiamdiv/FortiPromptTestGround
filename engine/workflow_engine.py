...
class WorkflowEngine:
    ...
    async def _execute_with_streaming(
        ...
        langgraph_config = {"configurable": runtime_config}
...
# class WorkflowEngine:
#     \"\"\"
#     The core orchestrator that executes a compiled state graph with middleware injection.

#     The WorkflowEngine wraps the underlying graph execution (e.g., LangGraph's `.astream()`) 
#     to provide a robust lifecycle hooks system. It handles initial state creation, 
#     merging state deltas (updates) emitted by nodes, and triggering before/after 
#     middlewares at both the run-level and step-level.

#     Attributes:
#         graph (CompiledGraph): The compiled executable state graph.
#         middlewares (List[BaseMiddleware]): A list of middleware instances to trigger during execution.
#         active_runs (Dict[str, Dict[str, Any]]): Internal tracking of runs currently being processed.
#     \"\"\"
# """
...
#     def __init__(self, compiled_graph:CompiledStateGraph, middlewares: List[BaseMiddleware] = None):
#         self.graph:CompiledStateGraph = compiled_graph
#         self.middlewares = middlewares or []
#         self.active_runs: Dict[str, Dict[str, Any]] = {}
...
#         # [FIXED]: Pass runtime_config to LangGraph via the "configurable" key
#         langgraph_config = {"configurable": runtime_config}
